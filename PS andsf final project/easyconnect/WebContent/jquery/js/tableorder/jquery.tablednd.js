 ...
 * @param tableId
 * Set Up for first and last row that not drag and drop
 */
function setTableDragAndDrop(tableId){
	// Remove Drag and Drop functionality from first column
	$("#"+tableId+" tr:first").addClass("nodrop nodrag");
	// Add message to last 
	$("#"+tableId+" tr:last").after("<tr><td colspan='100%' class='small-text-grey'>Note : Drag and Drop table rows for changing the order.</td></tr>");
	$("#"+tableId+" tr:last").addClass("nodrop nodrag");
	// Apply table Drag and Drop Functionality
	$("#"+tableId).tableDnD();
...
function saveOrder(id, type, responseUrl) {
	createNewForm("TableOrderForm","changeTableOrder.do");
	$("input:hidden[name='"+id+"']").each(function() { 
		 $("#TableOrderForm").append("<input type='hidden' name='ids' value='"+$(this).val()+"'>");
	});
	$("#TableOrderForm").append("<input type='hidden' name='className' value='"+type+"'>");
	$("#TableOrderForm").append("<input type='hidden' name='responseUrl' value='"+responseUrl+"'>");
